...
import { updateLeadStatus, markLeadRead, deleteLead } from "./actions";
...
export function LeadRow({
  ...
  const toggle = () => {
    setOpen((o) => !o);
    if (!lead.isRead) startTransition(() => markLeadRead(lead.id));
  };

  return (
    <div className="bg-white rounded-2xl border border-border-gray overflow-hidden">
      <button onClick={toggle} className="w-full flex items-center justify-between px-5 py-4 text-left">
        <div className="flex items-center gap-3 min-w-0">
          {!lead.isRead && <span className="w-2 h-2 rounded-full bg-focus-red shrink-0" />}
        ...
        <div className="px-5 pb-5 border-t border-border-gray pt-4 space-y-3">
          {lead.instagram && (
            <p className="text-sm text-foreground/70">
              <span className="text-foreground/40">Instagram:</span> {lead.instagram}
            </p>
          )}
          {lead.message && (
            <p className="text-sm text-foreground/70 leading-relaxed">
              <span className="text-foreground/40 block mb-1">Xabar:</span> {lead.message}
            </p>
          )}
